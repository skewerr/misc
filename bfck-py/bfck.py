...
def dec_pntr(_):
	...
	if dat_indx != 0:
		...
		raise Exception("Attempt to go to cell #-1")
...
while ins_indx < len(ins_tape):
	...
	print("   Cells:", '\t'.join(["{:04d}".format(_) for _ in dat_tape]))
	...
	if ins_indx != old_indx + 1 and ins_indx != old_indx:
		while old_indx != ins_indx:
			old_indx -= 1
			print("\033[5;1H", end="")
			print("    Next:", ins_prep(old_indx, 20), "\033[34m" +
				ins_tape[old_indx] + "\033[0m", ins_tape[old_indx+1:old_indx+21],
				" " * 20)

			time.sleep(p_sleep)
	else:
		print("    Next:", ins_prep(ins_indx, 20), "\033[31m" +
			(ins_tape[ins_indx] if ins_indx < len(ins_tape) else "HALT") + "\033[0m",
			ins_tape[ins_indx+1:ins_indx+21], " " * 20)
